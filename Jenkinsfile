pipeline {
    ...
    environment {
        NEXUS_URL = 'http://3.148.247.226:8081/' 
        NEXUS_REPO_JAR = 'my-maven-releases'     
        ARTIFACT_PATH = 'target/backend-0.0.1-SNAPSHOT.jar'
        GITHUB_REPO = 'https://github.com/AJAYMULAKALAPALLI/project'
        BRANCH = 'master'                       
        DEPLOYMENT_FILE_PATH = 'manifests/deployment.yml' 
        BACKEND_DIR = 'backend'
        DOCKER_IMAGE_NAME = 'backend-ap'
        DOCKER_HUB_USER = 'ajaymulakalapalli'
        VERSION= "1.0.0"
        TIMESTAMP = new Date().format("yyyyMMddHHmmss")
        IMAGE_TAG = "ajaymulakalapalli/backend-app:${TIMESTAMP}"
    }
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-cred', usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD')]) {
                    sh """
                        ...
                        git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/AJAYMULAKALAPALLI/project.git HEAD:${BRANCH}
                    """
                }
            }
        }
